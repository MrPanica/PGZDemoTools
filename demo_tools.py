...
I18N.ru.ticksShort='тики';I18N.en.ticksShort='ticks';I18N.ru.downloadArchive='Скачать всех .zip';I18N.en.downloadArchive='Download all .zip';I18N.ru.povPlayer='POV игрок';I18N.en.povPlayer='POV player';I18N.ru.freeCamera='Свободная камера';I18N.en.freeCamera='Free camera';I18N.ru.freeCameraNote='Экспорт без POV-команд: управление камерой доступно в демо-плеере.';I18N.en.freeCameraNote='Export without POV commands: camera controls are available in the demo player.';I18N.ru.povPreserved='Исходный POV сохраняется при обычном монтаже.';I18N.en.povPreserved='The original POV is retained in a normal montage.';I18N.ru.map='Карта';I18N.en.map='Map';I18N.ru.server='Сервер';I18N.en.server='Server';I18N.en.findPlayer='Find player';I18N.ru.demoPlayers='Игроки в демке';I18N.en.demoPlayers='Demo players';
...
function renderPov(){const pov=state.meta?.kind==='POV';$('#povControls').classList.toggle('hidden',!pov);if(!pov)return;const name=state.meta.client||'POV';$('#povPlayer').innerHTML=`<option>${esc(name)}</option>`;$('#povNote').textContent=$('#freeCamera').checked?tr('freeCameraNote'):tr('povPreserved')}
...
function syncSelection(fromInput=false){const m=state.meta;if(!m)return;let a=+$('#startRange').value,b=+$('#endRange').value;if(a>=b){if(fromInput&&document.activeElement===$('#startRange'))a=Math.max(0,b-1);else b=Math.min(m.ticks,a+1);$('#startRange').value=a;$('#endRange').value=b}$('#selection').style.left=(a/m.ticks*100)+'%';$('#selection').style.right=(100-b/m.ticks*100)+'%';$('#startTime').value=(a/m.tickRate).toFixed(2);$('#endTime').value=(b/m.tickRate).toFixed(2);$('#startTick').textContent=`· ${a.toLocaleString(locale())} ${tr('ticksShort')}`;$('#endTick').textContent=`· ${b.toLocaleString(locale())} ${tr('ticksShort')}`;saveSession()}
...
renderChat=()=>{rawRenderChat();if(!state.eventFilters)return;const events=state.events.filter(event=>EVENT_STYLE[event.kind]);document.querySelectorAll('#chatRows [data-event]').forEach(row=>row.hidden=!state.eventFilters.has(events[+row.dataset.event]?.kind))};
const rawDraw=draw;
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parsed.path == "/api/edit":
                ...
                free_camera = bool(payload.get("freeCamera")) and session["info"]["kind"] == "POV"
                default = session["info"]["path"].stem + ("-freecam" if free_camera else "-edit")
                ...
                write_edit(session["info"], ranges, target, free_camera=free_camera)
...
def main():
    ...
    try:
        if args.command in (None, "serve"):
            ...
            write_edit(info, [(start, end)], target, free_camera=args.free_camera)
            print(target)
